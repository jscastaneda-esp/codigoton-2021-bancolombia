...
ALTER TABLE `account`
ADD CONSTRAINT FOREIGN KEY (`client_id`) REFERENCES `client` (`id`);

CREATE INDEX `type` ON `client` (`type`);
CREATE INDEX `location` ON `client` (`location`);
CREATE INDEX `type-location` ON `client` (`type`, `location`);
